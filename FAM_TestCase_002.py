...
Flow:
  ...
  Step 4: Select Transaction Type = ACTUAL, Billing Period = 2026/02/01 - 2026/02/28
  Step 5: Open "View Document Summary" for agreement RJILDL_AIRTKN_ACTUAL_OFF_E_SO
...
def capture_invoice_from_dialog(page, context):
    ...
    try:
        usage_sum = frame.locator("td").filter(has_text="Sum : 178,590").first
        if usage_sum.count() == 0:
            usage_sum = frame.get_by_text("178,590", exact=True).first
        invoice["Usage Sum"] = usage_sum.inner_text().replace("Sum : ", "").strip()
    except Exception:
        invoice["Usage Sum"] = None

    try:
        tax_cell = frame.locator("td").filter(has_text="Sum : 12,033.40").first
        if tax_cell.count() == 0:
            tax_cell = frame.get_by_text("12,033.40", exact=True).first
...
def run():
    ...
    with sync_playwright() as p:
        ...
        page.get_by_text("ACTUAL", exact=True).click()
        page.wait_for_load_state("networkidle")
